...
mlflow_data = pd.read_pickle("reports/Streamlit/mlflow_database")
...
if page == "Introduction":  # Match with "pages"-entries
    ...
    col1.markdown("""
        ...
        - **T wave:** Represents ventricular repolarisation. This is when the ventricles recover and prepare for the next heartbeat.
                  """)
    col2.image("figures\Heartbeat_scheme.png",
...
if page == "Available Data":  # Match with "pages"-entries
    ...
    st.dataframe(pd.DataFrame({"Setting": measurement_setup.keys(),
              "Value": measurement_setup.values()}), hide_index=True, width=750)
    ...
    st.dataframe(dataset_comparision, hide_index=True, width=750)
...
if page == "Modelling":
    ...
    _, center, _ = st.columns(3)
    center.image("figures\\model_figures\\" + net_names[index] + ".jpg")
...
if page == "Modelling Results":
    ...
    if show_content:
        index = st.number_input("# Training History for index:",
                                    min_value=0, max_value=mlflow_data["index"].max(), value=None,
                                    placeholder="Insert Index-Number of Training", step=1)
...
if page == "Live Test":
    ...
    area = st.plotly_chart(fig, selection_mode="box", on_select="rerun")
